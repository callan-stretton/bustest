  ...
  renderServices () {
    ...
    this.markers = this.state.services.filter(service => service.Direction == (this.state.isInbound ? 'Inbound' : 'Outbound')).map((service) => {
      return new google.maps.Marker({
        ...
        title: (service.HasStarted === true) ? (service.BehindSchedule === true) ? (service.DelaySeconds < 60) ? 'Late by ' + moment('1900-01-01 00:00:00').add(service.DelaySeconds, 'seconds').format('ss') + ' seconds' : (service.DelaySeconds === 60) ? 'Late by 1 minute' : (service.DelaySeconds < 3600) ? 'Late by ' + moment('1900-01-01 00:00:00').add(service.DelaySeconds, 'seconds').format('mm:ss') + ' minutes' : 'Late by over an hour' : (service.DelaySeconds === 0) ? 'On Time' : (Math.abs(service.DelaySeconds) < 60) ? 'Early by ' + moment('1900-01-01 00:00:00').add(Math.abs(service.DelaySeconds), 'seconds').format('ss') + ' seconds' : (Math.abs(service.DelaySeconds) === 60) ? 'Early by 1 minute' : (Math.abs(service.DelaySeconds) < 3600) ? 'Early by ' + moment('1900-01-01 00:00:00').add(Math.abs(service.DelaySeconds), 'seconds').format('mm:ss') + ' minutes' : 'Early by over an hour!' : 'Not in service'
      })
    })
    if (this.stops) {
