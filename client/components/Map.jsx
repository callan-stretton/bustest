  ...
  renderServices () {
    ...
    this.markers = this.state.services.filter(service => service.Direction == (this.state.isInbound ? 'Inbound' : 'Outbound')).map((service) => {
      return new google.maps.Marker({
        ...
        title: (service.BehindSchedule === true) ? (service.DelaySeconds < 60) ? 'Running ' + moment('1900-01-01 00:00:00').add(service.DelaySeconds, 'seconds').format('ss') + ' seconds late' : (service.DelaySeconds === 60) ? 'Running 1 minute late' : (service.DelaySeconds < 3600) ? 'Running ' + moment('1900-01-01 00:00:00').add(service.DelaySeconds, 'seconds').format('mm:ss') + ' minutes late' : 'Running over an hour late' : (service.DelaySeconds === 0) ? 'On Time' : (Math.abs(service.DelaySeconds) < 60) ? 'Ahead of schedule by ' + moment('1900-01-01 00:00:00').add(Math.abs(service.DelaySeconds), 'seconds').format('ss') + ' seconds' : (Math.abs(service.DelaySeconds) === 60) ? 'Ahead of schedule by 1 minute' : (Math.abs(service.DelaySeconds) < 3600) ? 'Ahead of schedule by ' + moment('1900-01-01 00:00:00').add(Math.abs(service.DelaySeconds), 'seconds').format('mm:ss') + ' minutes' : 'Ahead of schedule by over an hour!'
      })
    })
    if (this.stops) {
